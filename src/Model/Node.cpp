...
void Node::SetGeom(unsigned int nodeId_0, std::vector<double>& xyz) {
	Index      = nodeId_0;
	XYZ[0] = xyz[0]; XYZ[1] = xyz[1]; XYZ[2] = xyz[2];
}
void Node::SetFixConstraints(const std::vector<unsigned int>& dofs_0) {
	for (unsigned int d : dofs_0) {
		assert(d < NDF_MAX);
		bcode[d] = 1;
	}
}
void Node::SetPreDispConstraints(unsigned int dof_0, double value) {
...
// 只有激活的自由自由度才编号
void Node::GenerateNodeEquation(unsigned int &NEQ) {
...
// 只有激活的自由自由度才写入结果
void Node::UpdateNodeDisplacement(const std::vector<double> &disp) {
	for (unsigned int i = 0; i < NDF_MAX; i++) {
		if (IsDofActive(i) && bcode[i] == 0) {
